...
type EvalFunction = (context: FieldEvalContext) => { result: boolean; log: string };
...
function getRuleEval(ruleId: string, fieldRules: FieldRulesState): EvalFunction {
    const rule = fieldRules.rules.entities[ruleId];

    const evaluate = (context: FieldEvalContext) => {
...
function buildGroupEval(groupId: string, fieldRules: FieldRulesState): EvalFunction {
...
export function getFieldsScoreEval(
    fieldIds: string[],
...
export function getFieldScores(
    fieldEvals: FieldScoreEval[],
    context: FieldEvalContext
): Record<string, number> {
    const scores: Record<string, number> = {};

    for (const fieldEval of fieldEvals) {
        const evaluateResult = fieldEval.evaluate(context);

        console.log(evaluateResult.log);

        if (evaluateResult.result) {
            const score = fieldEval.getScore();

            if (!scores[fieldEval.fieldId]) {
                scores[fieldEval.fieldId] = 0;
            }

            scores[fieldEval.fieldId] += score;
...
const context: FieldEvalContext = {
    'sentiment-field': { sentiment: 'POSITIVE' },
    'emotion-field': { emotion: 'JOY' }
};

const scoringTrees = getFieldsScoreEval(['sentiment-field', 'emotion-field'], fieldRules);
